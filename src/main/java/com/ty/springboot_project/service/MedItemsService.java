...
public class MedItemsService {
	...
	public ResponseEntity<ResponseStructure<MedItems>> saveMedItems(MedItems medItems, int medOrder_id) {
		...
		if (medOrder != null) {
			...
			throw new IdNotFoundException("No MedOrder is present for the given medOrder id");
	}
	...
	public ResponseEntity<ResponseStructure<MedItems>> deleteMedItems(int items_id) {
		ResponseStructure<MedItems> structure = new ResponseStructure<>();
		MedItems MedItems2 = medItemsDao.getMedItemsById(items_id);
		if (MedItems2 != null) {
			structure.setMessage("Successfully deleted MedItems");
			structure.setStatus(HttpStatus.OK.value());
			structure.setData(medItemsDao.deleteMedItems(items_id));
			return new ResponseEntity<ResponseStructure<MedItems>>(structure, HttpStatus.OK);
		} else
			throw new IdNotFoundException("No MedItem is present for the given meditem id");

	}

	public ResponseEntity<ResponseStructure<MedItems>> getMedItemsById(int items_id) {
		ResponseStructure<MedItems> structure = new ResponseStructure<>();
		MedItems MedItems2 = medItemsDao.getMedItemsById(items_id);
		if (MedItems2 != null) {
